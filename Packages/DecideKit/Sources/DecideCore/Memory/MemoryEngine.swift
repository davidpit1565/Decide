...
/// A preference DECIDE has learned from the user's actual decisions.
...
/// A preference DECIDE would like to remember, pending the user's consent.
...
public enum MemoryEngine {
    ...
    /// The keys the question engine treats as already known, so DECIDE does not ask
